...
function EventsPage() {
    ...
    useEffect(() => {
        ...
        const FindMovies = async() =>{
            setProgress(10);
            let url = `https://api.themoviedb.org/3/movie/now_playing?api_key=${process.env.REACT_APP_MOVIE_KEY}&page=${pageNo}`
    ...
    const fetchMoreData = async () =>{
        setPage(pageNo+1);
        let url = `https://api.themoviedb.org/3/movie/top_rated?api_key=${process.env.REACT_APP_MOVIE_KEY}&page=${pageNo}`
            let data = await fetch(url);
            let parsedData = await data.json();
...
export default MainLayoutHoc(EventsPage);